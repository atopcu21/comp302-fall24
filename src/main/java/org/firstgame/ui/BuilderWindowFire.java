...
public class BuilderWindowFire extends JPanel implements MouseListener, MouseMotionListener {
    private static BuilderWindowFire instance;
    private GameObject currentItem;
    private List<GameObject> placedObjects = new ArrayList<>();
    private JLabel levelLabel;

    private BuilderWindowFire() {
        ...
        levelLabel = new JLabel("Building Fire Hall");
    ...
    public static BuilderWindowFire getInstance() {
        if(instance == null) {
            instance = new BuilderWindowFire();
        }
        return instance;
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        try {
            ...
            nextButton = ImageIO.read(new File(PLAY_BUTTON_SPRITE));
        } catch (Exception e){
            // ignored
        }
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if(clickPoint.x >= 925 && clickPoint.x <= 925 + 70 && clickPoint.y >= 582 && clickPoint.y <= 582 + 36 && placedObjects.size() >= 17) {
    ...
    public void goToNextLevelBuilder() {
        ...
        RokueLikeGame.getInstance().addGameObjects(placedObjects, "Fire");
        RokueLikeGame.getInstance().initGame();
    }
    ...
    private void randomlyFill() {
        ...
        for (int i = 0; i < 17; i++) {
            ...
            if (i < 10) {
                gameObject = new GameObject(new WorldPosition(i + 1, i), selectedSprite);
            } else if(i < 13) {
                gameObject = new GameObject(new WorldPosition(i - 1, 12 -i), selectedSprite);
            }
            else{
                gameObject = new GameObject(new WorldPosition(i - 12, -8 +i), selectedSprite);
            }
